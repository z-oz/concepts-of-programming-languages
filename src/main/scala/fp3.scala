...
object fp3 {
  ...
  def allEqual (xs : List[Int]) : Boolean = {
    ...
    xs match {
    	...
    	case x::xs   => if (x == xs.head) allEqual(xs.tail)
                      else return false
	  }
  }
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    ...
    xs match{
    	case Nil     => Nil
    	case x::Nil  => List(x)
    	case x::xs   => if (x == xs.head) removeDupes1(xs)
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    def aux [X] (x:X, ys:List[X], i:Int) : List[(Int, X)] = ys match {
  		case List() => 	{
  		  if (x !=null) (i, x) :: List()
  		  else List()
  		}
  		case y :: ys =>	{
    		if (y != x) (i, x) :: aux(y, ys, 1)
    		else aux(y, ys,i + 1)
  		}
	}

	if(xs.length > 0) aux(xs.head, xs.tail, 1)
	else List()
  }
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
    ...
    xs match {
	    case Nil => return true
	    case x::xs => if (member(x,xs)) return false
                    else allDistinct(xs)
    }
  }
}
