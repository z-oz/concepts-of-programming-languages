...
object fp2 {
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    def f(s : String, x: String) : String = {s + x + term}
      xs match {
      	case Nil => ""
      	case x::xs => foldLeft(xs,term,f)
    }
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    // TODO: Provide definition here.
    def f(s: String, x: String) : String = {delimiter + s + x}
      xs match {
        case Nil => ""
        case x::xs => x + foldRight(xs,delimiter,f)
    }
  }
  ...
  def firstNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    ...
    xs match {
      ...
      case x::xs => if (x < a) firstNumGreaterThan(a,xs)
                    else x
    }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    def auxiliary(xs: List[Int], e: Int): Int = {
      xs match {
        case Nil => throw new RuntimeException()
        case x::xs => if (x >= a) e
                      else auxiliary(xs,e + 1)
      }
    }
    auxiliary(xs, 0)
  }
