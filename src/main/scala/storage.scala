...
object storage {
  ...
  val getAndSet : Int => (() => Int, Int => Unit) = {
    ...
    (n : Int) => {
      ...
      def set_pair : Int => Unit ={
        (x_num : Int) => keep_track = x_num
      }
      (get_pair, set_pair)
    }
  }
  ...
  val getAndSetSpy : () => (() => Int, Int => (() => Int, Int => Unit)) = {
    ...
    () => {
      var num = 0
      var keep_track = 0
      val getAndSetSpy : Int => (() => Int, Int  => Unit ) ={
          ...
          var set_pair: Int => Unit = {
            (x_num: Int) => { keep_track = keep_track + 1; num = x_num}};
            (get_pair,set_pair)
          }

        }
        var test : () => Int = { () => keep_track}
        var test2 : Int => (() => Int, Int => Unit) = {getAndSetSpy}
        (test,test2)
